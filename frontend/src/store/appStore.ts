...
export const useAppStore = create<AppState>((set, get) => ({
  ...
  showOnboarding: !localStorage.getItem('solarscope-onboarded'),
  isReportOpen: false,
  ...
  setShowOnboarding: (v) => {
    if (!v) localStorage.setItem('solarscope-onboarded', 'true');
    set({ showOnboarding: v });
  },
  setReportOpen: (v) => set({ isReportOpen: v }),
  updateSettings: (s) => set(s),
  reset: () => set({
    selectedLat: null, selectedLon: null, selectedBuilding: null,
